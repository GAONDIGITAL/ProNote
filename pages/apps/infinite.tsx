...
const infinite = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Infinite'));
    });
    ...
    // 모바일에서는 InfiniteScroll의 useWindow를 true로 설정하기위해 사용하였으나 PerfectScrollbar 사용으로 취소함
    // const [isMobile, setIsMobile] = useState(false);

    // useEffect(() => {
    //     const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);

    //     if (isMobile) setIsMobile(true); else setIsMobile(false);
    // }, []);
    ...
    const notesLoad = async () => {
        if (session) {
            ...
            try {
                const axiosData = { module: 'notesLoadInfinite', user: session?.user.id, page: page };
                ...
                >
                    <div className="flex h-full flex-col pb-16">
                        <div className="flex items-center text-center">
                            ...
                            <h3 className="text-lg font-semibold ltr:ml-3 rtl:mr-3">Infinite Scroll</h3>
                        </div>
                    </div>
                </div>
